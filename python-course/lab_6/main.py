...
("Логика", [("Иванов", 10), ("Петров", 15), ("Сидоров", 6), ("Жуков", 15), ("Noname", 0)])]
...
scores = lambda transformed_data: list(map(lambda name: (name, score_of_person(transformed_data)(name)), get_names(transformed_data)))
...
def do_it(transformed_data):
    return list(map(lambda d: (first(d), to_mark(second(d) / max_score(transformed_data))), transformed_data))

print()
print(do_it(scores(transform_to_list(data))))
